...
$fineStmt = $conn->prepare('SELECT reason, amount FROM fines WHERE customer_id = ?');
?>
                        ...
                        <tr>
                            ...
                            <td>
                                <button class="action-btn"
                                    onclick="toggleDetails(<?= (int) $row['customer_id']; ?>)">Details</button>
                            </td>
                        </tr>
                        <tr>
                            <td colspan="9">
                                <div id="details-<?= (int) $row['customer_id']; ?>" class="details-container">
                                    <div class="details-section">
                                        <h3>Customer Details</h3>
                                        <p><strong>Name:</strong> <?= htmlspecialchars($row['name']); ?></p>
                                        <p><strong>Email:</strong> <?= htmlspecialchars($row['email']); ?></p>
                                        <p><strong>Phone:</strong> <?= htmlspecialchars($row['phone']); ?></p>
                                        <p><strong>NIC:</strong> <?= htmlspecialchars($row['nic_number']); ?></p>
                                    </div>
                                    <div class="details-section">
                                        <h3>Fine Details</h3>
                                        <?php
                                        ...
                                        $totalFine = 0;
                                        if ($fineStmt) {
                                            $customerRef = (int) $row['customer_id'];
                                            $fineStmt->bind_param('i', $customerRef);
                                            $fineStmt->execute();
                                            $fineResult = $fineStmt->get_result();
                                            while ($fine = $fineResult->fetch_assoc()) {
                                                echo '<p><strong>Fine:</strong> $' . number_format((float) $fine['amount'], 2) . ' (Reason: ' . htmlspecialchars($fine['reason']) . ')</p>';
                                    ...
                                    <div class="details-section">
                                        ...
                                        <p><strong>Full Cost:</strong>
                                            $<?= number_format((float) $row['total_fees'] + $totalFine, 2); ?></p>
                                    </div>
                                </div>
                            </td>
                        </tr>
