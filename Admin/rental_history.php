                        ...
                        <tr>
                            ...
                            <td>
                                <button class="action-btn"
                                    onclick="toggleDetails(<?= (int) $row['history_id']; ?>)">Details</button>
                                <a href="generate_invoice_image.php?history_id=<?= (int) $row['history_id']; ?>"
                                    class="action-btn">Download JPG</a>
                            </td>
                        </tr>
                        <tr>
                            <td colspan="6">
                                <div id="details-<?= (int) $row['history_id']; ?>" class="details-container">
                                    ...
                                    <div class="details-section">
                                        <h3>Car Details</h3>
                                        <p><strong>Brand:</strong> <?= htmlspecialchars($row['car_brand']); ?></p>
                                        <p><strong>Model:</strong> <?= htmlspecialchars($row['car_model']); ?></p>
                                        <p><strong>Seats:</strong> <?= htmlspecialchars($row['seat_count']); ?></p>
                                        <p><strong>Max Speed:</strong> <?= htmlspecialchars($row['max_speed']); ?> km/h</p>
                                        <p><strong>Efficiency:</strong> <?= htmlspecialchars($row['km_per_liter']); ?> km/l
                                        </p>
                                    </div>
                                    <?php if (!empty($row['fine_details'])): ?>
                                        <div class="details-section">
                                            <h3>Fine Details</h3>
                                            <p><?= htmlspecialchars($row['fine_details']); ?></p>
                                        </div>
                                    <?php endif; ?>
                                    <?php if (!empty($row['additional_options'])): ?>
                                        <div class="details-section">
                                            <h3>Additional Options</h3>
                                            <p><?= htmlspecialchars($row['additional_options']); ?></p>
                                        </div>
                                    <?php endif; ?>
                                    <div class="details-section">
                                        <h3>Total Fees</h3>
                                        <p><strong>Total Fees:</strong>
                                            $<?= number_format((float) $row['total_fees'], 2); ?></p>
                                    </div>
                                </div>
                            </td>
                        </tr>
